...
Under the hood it’s multi-user SQLite, a FastAPI backend, and a React + Vite frontend. Each account keeps its own labels and events.
...
| `frontend-react/` | React + Vite source. Run `npm run dev` here during development. |
| `frontend-dist/` | Production build output (generated by `npm run build`, served by FastAPI). Gitignored. |
| `timelogger.db` | SQLite file (created at the **repository root** on first run). Gitignored. |
...
App runs at `http://localhost:5173`. Vite proxies all API calls to the backend automatically — no CORS config needed.

### Create an account

Open `http://localhost:5173` and click **Create an account**. Enter your display name, username, and a password (min 6 characters). The first registered user is automatically made admin.
...
```bash
sudo apt install -y python3 python3-venv python3-pip nodejs npm nginx certbot python3-certbot-nginx
```

### 4. Clone the app, install dependencies, and build the frontend
...
sudo -u www-data bash -c 'cd /opt/lifelogger/frontend-react && npm install && npm run build'
```
